...
CLIENT_DIR = os.path.join(os.path.dirname(__file__), "..", "scrips", "fusion360gym", "client")
...
class ReplEnv(GymEnv):
    ...
    def set_target(self, target_file):
        ...
        # ---- Start: Add print statements for verification ----
        print("--- Server Response Data Keys ---")
        if "data" in response_json:
            print(list(response_json["data"].keys()))
            if "exported_brep_path" in response_json["data"]:
                exported_path = response_json["data"]["exported_brep_path"]
                print(f"Exported B-Rep Path: {exported_path}")
            else:
                print("Exported B-Rep Path: Not found in response.")
        else:
            print("No 'data' key found in response.")
        print("---------------------------------")
        # ---- End: Add print statements ----
