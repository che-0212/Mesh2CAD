...
REGRAPHNET_DIR = os.path.join(os.path.dirname(__file__), "..", "train")
...
class AgentSupervised(Agent):
    ...
    def __init__(self, agent, syn_data):
        super().__init__()
        if agent not in ["gcn", "mlp"]:
            print(f"Error: Agent {agent} not supported. Only 'gcn' and 'mlp' are supported.")
            exit()
